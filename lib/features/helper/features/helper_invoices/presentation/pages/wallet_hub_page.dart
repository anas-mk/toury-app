    ...
    return MultiBlocProvider(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          title: const Text('Wallet Hub', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          bottom: TabBar(
            ...
            child: ListView(
              ...
              children: [
                ...
                const FadeInSlide(
                  ...
                  child: Text('Summary', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
                ),
                ...
                FadeInSlide(
                  ...
                  child: Row(
                    children: [
                      Expanded(child: _MiniStatCard(label: 'Weekly', amount: state.earnings.week, color: const Color(0xFF6C63FF))),
                      const SizedBox(width: 12),
                      Expanded(child: _MiniStatCard(label: 'Monthly', amount: state.earnings.month, color: const Color(0xFFFFAB40))),
                    ],
                  ),
                ),
                const SizedBox(height: 24),
                const FadeInSlide(
                  ...
                  child: Text('Recent Payouts', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
                ),
                const SizedBox(height: 12),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: const Color(0xFF00C896).withOpacity(0.3), blurRadius: 20, offset: const Offset(0, 10)),
        ],
      ),
      child: Column(
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(color: const Color(0xFF00C896).withOpacity(0.1), shape: BoxShape.circle),
            child: const Icon(Icons.add_rounded, color: Color(0xFF00C896), size: 18),
          ),
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(item.travelerName, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
                Text(DateFormat('MMM d, yyyy').format(item.date), style: const TextStyle(color: Colors.white38, fontSize: 11)),
              ],
            ),
          ),
          Text('+\$${item.amount.toStringAsFixed(2)}', style: const TextStyle(color: Color(0xFF00C896), fontWeight: FontWeight.bold, fontSize: 16)),
        ],
      ),
    );
  }
        ...
        child: Row(
          children: [
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('#${invoice.invoiceNumber}', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(height: 2),
                Text(invoice.userName, style: const TextStyle(color: Colors.white38, fontSize: 12)),
              ],
            ),
            const Spacer(),
            Column(
              ...
              children: [
                Text('${invoice.currency} ${invoice.totalAmount.toStringAsFixed(2)}', style: const TextStyle(color: Color(0xFF00C896), fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Container(
