    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData == null || responseData is! Map<String, dynamic>) {
          debugPrint('[InstantSearch] searchInstantHelpers: data is null or not a Map — returning []');
          return [];
        }
        final helpersRaw = responseData['helpers'];
        if (helpersRaw == null || helpersRaw is! List) {
          debugPrint('[InstantSearch] searchInstantHelpers: helpers missing or not a List — returning []');
          return [];
        }
        debugPrint('[InstantSearch] helpers count: ${(helpersRaw as List).length}');
        return helpersRaw
            .whereType<Map<String, dynamic>>()
            .map(HelperBookingModel.fromJson)
            .toList();
      } else {
        throw ServerException(response.data['message'] ?? 'Search failed');
      }
