...
class _LocationPickerPageState extends State<LocationPickerPage> {
  ...
  final Map<String, _ReverseCacheValue> _reverseCache = {};
  String? _lastReverseKey;
  static const Duration _reverseCacheTtl = Duration(minutes: 3);
  ...
  void _scheduleReverseGeocode(_LL latLng, {bool immediate = false}) {
    ...
    if (cached != null &&
        DateTime.now().difference(cached.at) <= _reverseCacheTtl) {
      _applyReverseResult(latLng, cached.result);
      _lastReverseKey = key;
  ...
  Future<void> _reverseGeocode(_LL latLng) async {
    ...
    try {
      ...
      _applyReverseResult(latLng, r);
      _reverseCache[key] = _ReverseCacheValue(result: r, at: DateTime.now());
      _lastReverseKey = key;
      _resolvingVN.value = false;
    } catch (_) {
      if (!mounted || token.isCancelled) return;
  ...
  String _reverseKey(_LL latLng) =>
      '${latLng.latitude.toStringAsFixed(4)},${latLng.longitude.toStringAsFixed(4)}';

  void _applyReverseResult(_LL latLng, NominatimResult? r) {
    if (r != null) {
...
class _ReverseCacheValue {
  const _ReverseCacheValue({required this.result, required this.at});
