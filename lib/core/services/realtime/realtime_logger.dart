...
class RealtimeLogger {
  ...
  void log(
    ...
    _stream.add(entry);
    // Pass #4 perf: only ship `debugPrint` to the platform thread when
    // we're actually in a debug build. The in-app diagnostics page still
    // works in profile/release because we keep the in-memory buffer +
    // broadcast stream above this guard.
    if (kDebugMode) {
